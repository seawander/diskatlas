...
Any paper whose citation key is already defined by hand in the sibling
local.bib is omitted from the output, so that a paper that is both an
atlas record and a hand-added reference (e.g. a survey's series-I paper)
is not emitted twice and does not trigger a repeated-entry error.
...
def local_bib_keys(path):
    """Citation keys defined by hand in a sibling local.bib. Papers with these
    keys are skipped in the export so a reference that is both an atlas record
    and a hand-added entry is not written twice (which BibTeX flags as a
    repeated entry)."""
    if not path.exists():
        return set()
    return {m.group(1).strip() for m in
            re.finditer(r"@[A-Za-z]+\s*\{\s*([^,]+?)\s*,", path.read_text(errors="replace"))}
...
def main():
    ...
    # papers already defined by hand in local.bib are its responsibility;
    # skip them here so nothing is emitted (and cited) twice.
    local_keys = local_bib_keys(Path(args.out).resolve().parent / "local.bib")
    ...
    lines = ["%% Auto-generated by backend/export_bibtex.py -- do not edit by hand.",
             ...
             "%% Keys also present in local.bib are omitted here to avoid duplicates.", ""]
    skipped, written = [], 0
    ...
    print(f"wrote {args.out}: {written} entries", file=sys.stderr)
    if skipped:
        print(f"  skipped {len(skipped)} already in local.bib: "
