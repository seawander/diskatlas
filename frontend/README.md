...
| Facets (Sky only, collapsible; **collapsed by default on phones ≤640px**): Band / Missing / Facility (`fac_keys`, VLT⊃VLTI) / Instrument (`instr_key`, parent⊇children so `SPHERE`⊇`SPHERE/IRDIS`, likewise `SCExAO/*`) | `chipGroup()`, `filterSystems()`, `updateFacetVisibility()` |
