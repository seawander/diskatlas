...
| Views via top switcher: Sky / Coverage matrix / Tonight. **Re-clicking the active Coverage/Tonight tab returns to Sky** | `setView()`, `buildMatrix()`, `buildTonight()`/`computeTonight()` |
...
| Facets (Sky only): an always-visible **Filters** button (top-right, by the view tabs) folds/unfolds them and badges the active count; folded by default on small/touch screens (or the user's persisted choice). Phones open the block as a **bottom sheet** over the map; tablet/desktop fold it inline. Groups with **>8 chips** (Facility/Instrument/Survey) start folded to a `▸N` summary and expand on tap; active chips stay visible. Groups: Band / Content (continuum⧸line from the record `content` field; mirrored as down-select chips in the Coverage matrix + Tonight) / Missing / Facility (`fac_keys`, VLT⊃VLTI) / Instrument (`instr_key`, parent⊇children so `SPHERE`⊇`SPHERE/IRDIS`, likewise `SCExAO/*`) / Survey | `chipGroup()`, `filterSystems()`, `updateFacetVisibility()`, `updateToggleLabel()`, `catChipsHTML()` |
| **Relationship highlight** (`.rel`): facility↔instrument (selecting one lights the other; parent `SPHERE` lights its sub-instruments), and survey↔facility/instrument (selecting a survey lights the facilities/instruments that enabled it, and vice-versa). Related chips stay visible even inside a folded group | `updateRelHighlights()` |
